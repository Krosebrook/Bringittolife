...
export const PreviewToolbar: React.FC<PreviewToolbarProps> = ({
    ...
    return (
        <div className="bg-[#121214] px-4 py-3 flex items-center justify-between border-b border-zinc-800 shrink-0 z-50 relative">
            {/* Left: Controls */}
                    ...
                    <>
                        ...
                        <div className="flex items-center bg-zinc-900/50 rounded-md p-0.5 mr-2 border border-zinc-800/50">
                            <Tooltip content="Zoom Out" side="bottom">
                                <button onClick={onZoomOut} className="p-1.5 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 rounded transition-all">
                                    <MagnifyingGlassMinusIcon className="w-4 h-4" />
                                </button>
                            </Tooltip>
                            
                            <Tooltip content="Reset View" side="bottom">
                                <button onClick={onResetView} className="px-2 py-1 min-w-[3rem] text-[10px] font-mono text-zinc-400 hover:text-zinc-200 hover:bg-zinc-800 rounded transition-all">
                                    {Math.round(scale * 100)}%
                                </button>
                            </Tooltip>

                            <Tooltip content="Zoom In" side="bottom">
                                <button onClick={onZoomIn} className="p-1.5 text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800 rounded transition-all">
                            ...
                            <Tooltip content={isPanMode ? "Exit Pan Mode" : "Pan Tool"} side="bottom">
                                <button 
                        ...
                        {/* Device Toggles */}
                        <div className="flex items-center bg-zinc-900/50 rounded-md p-0.5 mr-3 border border-zinc-800/50">
                        ...
                        {creation.originalImage && (
                            <Tooltip content={showSplitView ? "Show App Only" : "Compare with Original"} side="bottom">
                                <button 
                        ...
                        <Tooltip content={isDragMode ? "Disable Element Dragging" : "Enable Element Dragging"} side="bottom">
                            <button
                                onClick={onToggleDragMode}
                                className={`p-1.5 rounded-md transition-all ${isDragMode ? 'bg-blue-900/30 text-blue-400 ring-1 ring-blue-500/50' : 'text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800'}`}
                            >
                        ...
                        <Tooltip content="Export Artifact (JSON)" side="bottom">
                            <button 
                        ...
                        <Tooltip content="New Upload" side="bottom">
                            <button 
                                onClick={onReset}
